...
function PersonProfile({ person, holidays, onBack }) {
  ...
  function getStatus(h) {
    if (dayjs(h.endDate).isBefore(today)) return { label: 'Completed', color: '#999' }
    if (dayjs(h.startDate).isAfter(today)) return { label: 'Upcoming', color: '#1976d2' }
      ...
      <div className="profile-body">
        ...
        <div className="profile-left">
          <div className="profile-avatar-large">
            {person.picture ? (
              <img src={URL.createObjectURL(person.picture)} alt={person.name} />
            ) : (
          ...
          <div className="allowance">
            ...
            <div className="allowance-row">
              <span>Total Days</span><span>{totalDays} days</span>
            </div>
            <div className="allowance-row">
              <span>Used</span><span>{usedDays} days</span>
            </div>
            <div className="allowance-row">
              <span>Remaining</span>
          ...
          <div className="profile-actions">
            <button className="btn-add-holiday">+ Add Holiday</button>
        ...
        <div className="profile-right">
          ...
          <div className="holiday-list">
            {filteredHolidays.length === 0 ? (
              ...
              filteredHolidays.map((h) => {
                ...
                const days = dayjs(h.endDate).diff(dayjs(h.startDate), 'day') + 1
                return (
                  <div key={h.id} className="holiday-card">
                    <div className="holiday-card-date">
                      <span className="month">{dayjs(h.startDate).format('MMM')}</span>
                      <span className="day-num">{dayjs(h.startDate).format('D')}</span>
                    </div>
                    <div className="holiday-card-info">
                      <div className="holiday-card-badges">
                        ...
                        {h.type && (
                          <span className="type-badge">{h.type}</span>
                        )}
                      </div>
                      <p className="holiday-card-dates">
                        {dayjs(h.startDate).format('dddd, MMMM D')} – {dayjs(h.endDate).format('dddd, MMMM D, YYYY')}
                      </p>
                      <p className="holiday-card-days">{days} day{days > 1 ? 's' : ''}</p>
                    </div>
                  </div>
                )
              })
          ...
          <div className="profile-summary">
            ...
            <div className="summary-card">
              <p className="summary-num">{holidays.filter(h => !dayjs(h.endDate).isBefore(today) && !dayjs(h.startDate).isAfter(today)).length}</p>
              <p className="summary-label">Active Holiday</p>
            </div>
            <div className="summary-card">
              <p className="summary-num">{holidays.filter(h => dayjs(h.startDate).isAfter(today)).length}</p>
